...
async function run(subclass){
    let res = await Database.makeQuery2("SELECT ShortName, LongName, Matnr FROM products WHERE Class LIKE '%Networking%' AND SubClass LIKE '%" + subclass + "%';")
...
if(process.argv.length !== 2){
    ...
    if(process.argv.includes("--with-subclass")){
